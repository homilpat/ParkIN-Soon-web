...
BASE_DIR   = r"C:\Users\asia\Desktop\KKH\SEMI_2_project"
IMG_SIZE   = (224, 224)
BATCH_SIZE = 8
SEED       = 42
N_FOLDS    = 5
...
# K-Fold OOF 예측값 수집
oof_preds  = np.zeros(len(sp_train_labels))
oof_labels = sp_train_labels.copy()

skf = StratifiedKFold(n_splits=N_FOLDS, shuffle=True, random_state=SEED)

for fold, (train_idx, val_idx) in enumerate(
        skf.split(sp_train_paths, sp_train_labels), 1):

    print(f"Fold {fold}/{N_FOLDS} 예측 중...")

    vl_paths  = sp_train_paths[val_idx]
    vl_labels = sp_train_labels[val_idx]

    val_df = pd.DataFrame({
        'filename': vl_paths,
        'class': vl_labels.astype(str)
    })
    ...
    # best_model은 K-Fold 학습 후 저장된 최고 성능 모델
    fold_preds = best_model.predict(val_gen, verbose=0).flatten()
    oof_preds[val_idx] = fold_preds

# holdout과 합치기
all_preds  = np.concatenate([oof_preds, preds])
all_labels = np.concatenate([oof_labels, labels])

np.save('y_pred_img.npy', all_preds)
np.save('y_true_img.npy', all_labels)

print(f"\n저장 완료")
print(f"OOF training: {len(oof_preds)}장")
print(f"Holdout:      {len(preds)}장")
print(f"합계:         {len(all_preds)}장")
print(f"PD={all_labels.sum()}장, HC={(all_labels==0).sum()}장")

best_model.save(
    r"C:\Users\asia\Desktop\KKH\SEMI_2_project\modeling\drawing_cnn_savedmodel"
)
