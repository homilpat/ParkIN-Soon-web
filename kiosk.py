...
# ── 브랜드 색상 ────────────────────────────────────────────────────────────────
C_GREEN  = "#02B638"   # 정상 / 긍정
C_ORANGE = "#F08700"   # 주의 / 경고
...
div[data-testid="stMarkdownContainer"] li {{ font-size:20px!important; }}
...
def progress(cur):
    ...
    for i, lbl in enumerate(steps):
        ...
        dc = C_GREEN if (done or active) else "#e2e8f0"
        lc = C_GREEN if active else ("#64748b" if done else "#cbd5e1")
        fw = "700" if active else "500"
        inn = "✓" if done else str(i + 1)
        tc = "white" if (done or active) else "#94a3b8"
        dots += f"""<div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1;">
            <div style="width:32px;height:32px;border-radius:50%;background:{dc};
                display:flex;align-items:center;justify-content:center;
                font-size:20px;font-weight:700;color:{tc};">{inn}</div>
            <div style="font-size:20px;color:{lc};font-weight:{fw};">{lbl}</div></div>"""
        if i < len(steps) - 1:
            lc2 = C_GREEN if done else "#e2e8f0"
            dots += f'<div style="flex:1;height:2px;background:{lc2};margin-top:16px;border-radius:2px;"></div>'
    st.markdown(
        f'<div style="background:white;border-radius:16px;padding:20px 24px;'
        f'margin-bottom:28px;box-shadow:0 2px 12px rgba(0,0,0,0.06);'
...
def preprocess_canvas(img_arr):
    ...
    comp = rgba[:, :, :3] * alpha + 255.0 * (1.0 - alpha)
...
if st.session_state.step == 0:
    char_path = APP_DIR / "character.png"
    char_b64 = get_img_base64(char_path)
    char_html = (
    f'<img src="data:image/png;base64,{char_b64}" '
    f'style="height:300px;object-fit:contain;'
    f'filter:drop-shadow(0 4px 16px rgba(0,0,0,0.25));" />'
    if char_b64 else ""
    )

    st.markdown(f"""
    <div style="background:linear-gradient(135deg,#1a3a5c,#02B638);
                border-radius:24px;padding:50px 40px;
                margin-bottom:32px;box-shadow:0 8px 32px rgba(2,182,56,0.25);
                display:flex;align-items:center;justify-content:space-between;
                gap:30px;">
        <div style="text-align:left;flex:1;">
            <div style="display:inline-block;background:rgba(255,255,255,0.15);
                        border:1px solid rgba(255,255,255,0.3);color:white;
                        border-radius:24px;padding:6px 16px;font-size:20px;
                        font-weight:600;margin-bottom:24px;">
                🔬 AI 기반 비침습적 조기 선별 도구
            </div>
            <div style="font-size:56px;font-weight:900;color:white;
            ...
            <div style="font-size:20px;color:rgba(255,255,255,0.8);
                        line-height:2.0;">
                후각 · 배변 · 손그림 세 가지 검사를 통해<br>
                파킨슨병 관련 이상 징후를 확인합니다.</div>
        </div>
...
<div style="background:#fffbeb;border:1.5px solid {C_YELLOW};border-radius:14px;
    ...
    st.markdown(f"""
<div style="background:white;border:1.5px solid #dbeafe;border-radius:16px;
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    progress(1)
    st.markdown(f"""
<div style="background:linear-gradient(135deg,#0f766e,{C_GREEN});border-radius:18px;
            padding:28px 32px;margin-bottom:28px;color:white;">
    <h2 style="margin:0 0 8px;font-size:26px;font-weight:800;">👃 Step 1. 후각 인지 테스트</h2>
    <p style="margin:0;opacity:0.88;font-size:20px;line-height:1.6;">
    스크래치 카드의 향기를 맡아보고,<br>선명하게 구별이 되면 ○, 잘 모르겠으면 ✗를 선택해주세요.</p>
</div>""", unsafe_allow_html=True)
...
<div style="background:white;border:1.5px solid #e2e8f0;border-radius:14px;
            padding:16px 10px 10px;text-align:center;margin-bottom:4px;">
    <div style="font-size:32px;margin-bottom:6px;">{icon}</div>
    <div style="font-size:20px;font-weight:600;color:{C_DARK};">{lbl}</div>
</div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    progress(2)
    st.markdown(f"""
<div style="background:linear-gradient(135deg,#7c3aed,{C_ORANGE});border-radius:18px;
            padding:28px 32px;margin-bottom:28px;color:white;">
    <h2 style="margin:0 0 8px;font-size:26px;font-weight:800;">🩺 Step 2. 배변 상태 체크</h2>
    <p style="margin:0;opacity:0.88;font-size:20px;line-height:1.6;">
    최근 한 달 동안의 증상에 대해 솔직하게 답변해주세요.</p>
</div>""", unsafe_allow_html=True)
    ...
    for i, (key, q, hint) in enumerate(qs):
        st.markdown(f"""
<div style="background:white;border:1.5px solid #ede9fe;border-radius:16px;
            padding:22px 24px 16px;margin-bottom:18px;
            box-shadow:0 2px 10px rgba(240,135,0,0.06);">
    <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:10px;">
        <div style="background:{C_ORANGE};color:white;border-radius:50%;
                    width:30px;height:30px;min-width:30px;
                    display:flex;align-items:center;justify-content:center;
                    font-size:14px;font-weight:700;margin-top:2px;">{i + 1}</div>
        <div>
            <div style="font-size:20px;font-weight:700;color:{C_DARK};line-height:1.5;">{q}</div>
            <div style="font-size:20px;color:#64748b;margin-top:4px;">{hint}</div>
        </div>
    </div>
</div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    progress(3)
    st.markdown(f"""
<div style="background:linear-gradient(135deg,#be185d,{C_ORANGE});border-radius:18px;
            padding:28px 32px;margin-bottom:24px;color:white;">
    <h2 style="margin:0 0 8px;font-size:26px;font-weight:800;">✍️ Step 3. 나선 그리기 테스트</h2>
    <p style="margin:0;opacity:0.88;font-size:20px;line-height:1.6;">
    손의 미세한 떨림과 운동 속도를 분석합니다.</p>
</div>
<div style="background:#fff7ed;border-left:4px solid {C_ORANGE};border-radius:10px;
            padding:14px 18px;margin-bottom:20px;font-size:20px;
            color:#7c2d12;line-height:1.8;">
    📌 <strong>그리는 방법</strong><br>
    왼쪽 가이드라인을 참고하여, 오른쪽 캔버스에
    <strong>안쪽 중심에서 바깥쪽 방향</strong>으로 나선을 그려주세요.
</div>""", unsafe_allow_html=True)

    cl, cr = st.columns([1, 1])
    with cl:
        st.markdown("**📋 참고 가이드**"); st.image(spiral_img(), width=400)
    with cr:
        st.markdown("**🖊️ 여기에 그려주세요**")
        cr_result = st_canvas(
            fill_color="rgba(0,0,0,0)", stroke_width=3,
            stroke_color=C_DARK, background_color="#FFFFFF",
            height=CANVAS_SIZE, width=CANVAS_SIZE,
            drawing_mode="freedraw", key="spiral_canvas", display_toolbar=True
        )
    ...
    if st.button("✅ 분석 결과 확인", use_container_width=True, type="primary"):
        if (cr_result.image_data is not None and cr_result.json_data is not None
            ...
            st.session_state.canvas_img = cr_result.image_data
            xa, ya, ta, ts = [], [], [], 0
